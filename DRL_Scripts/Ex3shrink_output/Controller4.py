...
class controller(Sofa.PythonScriptController):
    ...
    def initGraph(self, node):
            ...
            self.switch = 0
            self.breakPoint = 1
            self.csv = np.genfromtxt('/home/owen/Softwares/SOFADRL/scenes/ReducedScenes/Ex3shrink_output/results_Ex3_uniform_real.csv', delimiter=",")
    ...
    def onBeginAnimationStep(self, dt):
        ...
        if self.i>=20:
            ...
            #self.rand += 0.09/10000
            self.rand = self.csv[self.it][0]*0.01
            self.i = 0
    ...
    def onEndAnimationStep(self, dt):
        ...
        print(self.it)
        if self.it > len(self.csv)-1:
            self.pneu1Node.getRootContext().animate = False
            with open('pressurevsposition_uniform4_full.csv', 'wb') as f:
                wr = csv.writer(f)
                mylist = [x,y,p]
                array = np.array(mylist)
                transpose = array.T
